...
(function () {
    ...
    function myMove(opt) {
        ...
        if (typeof defaultOpt.effect == 'number') {
            arr = arr[defaultOpt.effect % arr.length].split('-');
            tempEffect = arr.length >= 2 ? cm[arr[0]][arr[1]] : cm[arr[0]];
        }

        for (var attr in defaultOpt.target) {
            if (defaultOpt.target.hasOwnProperty(attr)) {
                begin[attr] = utils.getCss(defaultOpt.id, attr);
                change = defaultOpt.target[attr] - begin[attr];
            }
        }
        ...
        defaultOpt.id.timer = setInterval(function(){
            if(time >= defaultOpt.duration){
                ...
                clearInterval(defaultOpt.id.timer);
                typeof defaultOpt.callback == 'function'? (defaultOpt.callback.call(defaultOpt.id)):void 0;
                return;
            }

            time +=20;
            for(var attr in begin ){
                if(begin.hasOwnProperty(attr)){
                   var curPos =  tempEffect(time,begin[attr],change[attr],defaultOpt.duration) ;
 ...
 * opt:{
 *      id:xxx(必传)
 *      target:{}  目标值 对象形式 支持多属性运动
  *     effect:0-8  默认匀速  运动形式 必须是数字
  *     duration:xxx  默认800毫秒  运动时间
